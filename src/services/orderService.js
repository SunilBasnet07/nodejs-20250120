...
const getOrdersByUser = async (userId, query) => {
    const filter = {}
    if (query.status) filter.status = query.status || PENDING_STATUS
    return await Order.find({ userId: userId }, filter).sort({ "createdAt": -1 }).populate("orderItem.product",).
        populate("userId", ["name", "email", "address", "number"]);
}
...
const confirmOrder = async (id, data) => {
 ...
 return await Order.findByIdAndUpdate(id,{status:CONFIRMED_STATUS},{new:true})  

}


export default { getAllOrders, createOrder, getOrdersByUser, updateOrderByStatus, getOrderById, checkoutOrder,confirmOrder }